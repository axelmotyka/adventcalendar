...
Von der Firma wird der Name, eine Adresse, die Anzahl der Räume zur Vermietung und die maximale Anzahl der möglichen Räume gespeichert.
Alle Attribute müssen gelesen werden können.
Die Größe der Räume (d. h. die Menge der Sitzplätze) wird in einem Feld gespeichert.
Die einzelnen Räume werden von Kunden gebucht, diese werden in einem Feld gespeichert.
Eine Firma wird mit ihrem Namen und der Adresse erzeugt, zudem werden alle notwendigen Werte gesetzt.
Es kann ein Raum (mit der Sitzplatzmenge) hinzugefügt werden.
Es muss eine Methode geben, die nach einem Raum mit einer gewünschten Größe sucht und dessen Nummer zurückgibt; findet sich kein Raum, wird -1 zurückgegeben.
Es muss eine Methode geben, die die Anzahl der freien (ungebuchten) Räume ermittelt und zurückgibt.
